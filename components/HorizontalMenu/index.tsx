...
const HorizontalMenu = ({ className = '', onItemChange, value, items, ...props }: Props): JSX.Element => {
	const selectedRef = useRef(null);

	function getAdditionalPadding() {
		if(value === 0) 
			return '+ 3px';
		else if(value + 1 === items.length)	
			return '- 3px';
		else
			return '';
	}
		...
		>
			...
			<div
				...
				style={{
					...
					left: `calc(${100 / items.length * value + '%'} ${getAdditionalPadding()})`,
				}}
			>
			</div>
		</div>
